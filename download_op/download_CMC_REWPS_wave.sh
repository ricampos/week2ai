...
########################################################################
# download_CMC_REWPS_wave.sh
#
# VERSION AND LAST UPDATE:
#   v1.0  09/03/2025
#
# PURPOSE:
#  Script to download Environment Canada / Canadian Meteorology Centre (CMC) wave ensemble forecast (regional)
#  Great Lakes
...
#   https://eccc-msc.github.io/open-data/msc-data/nwp_rewps/readme_rewps_en/
#
# USAGE:
#  Three input arguments must be entered: cycle time (00 or 12), time lag(days), and the output path.
#  Example:
#    bash download_CMC_REWPS_wave.sh 00 0 /home/ricardo/data/EnvCanada/wave
...
exec > >(tee -a "$WDIR/download_CMC_REWPS_wave_${DATE}${CHOUR}.log") 2>&1
...
for h in $fleads;do
  ...
  for i in "${!WVARS[@]}"; do
    ...
    echo " ======== REWPS Forecast: ${DATE} ${CHOUR}Z $h ${wv} ========"

    arqn="rewps.wave.${DATE}T${CHOUR}Z_"${h}"H_${wv}"

    FILE="$WDIR/REWPS_wave_${DATE}${CHOUR}.00.nc"
    ...
    wfile="${DATE}T${CHOUR}Z_MSC_REWPS-Great-Lakes_${wv}_Sfc_LatLon0.022x0.031_PT"${h}"H.grib2"
    wget -l1 -H -t1 -nd -N -np -erobots=off --tries=3 "${SERVER}/${DATE}/WXO-DD/model_rewps/great-lakes/grib2/${CHOUR}/${wfile}" -O "$WDIR/work_${DATE}${CHOUR}/${arqn}.grib2"
    ...
    if [ ${TE} -eq 0 ]; then
      ...
      for j in "${!ensbl[@]}"; do
        ...
        if [ "$j" -eq 0 ]; then
          ...
          ncrename -v ${wvarn},${wv} ${arqn}_${ne}.saux.nc ${arqn}_${ne}.nc
        else
          ncks -v "${wvarn}_$e" ${arqn}.saux.nc -o ${arqn}_${ne}.saux.nc
          ncrename -v ${wvarn}_${e},${wv} ${arqn}_${ne}.saux.nc ${arqn}_${ne}.nc
        fi
  ...
  for i in "${!ensbl[@]}"; do
    ...
    FILE="$WDIR/REWPS_wave_${DATE}${CHOUR}.${ne}.nc"
    cfile=$(checkfile "$FILE")
    if [ "$cfile" -eq 0 ]; then
    ...
    for j in "${!WVARS[@]}"; do
      wv="${WVARS[$j]}"
      if [ "$j" -eq 0 ]; then
        iname="rewps.wave.${DATE}T${CHOUR}Z_${h}H_${ne}.nc"
        mv "rewps.wave.${DATE}T${CHOUR}Z_${h}H_${wv}_${ne}.nc" "$iname"
      else
        src="rewps.wave.${DATE}T${CHOUR}Z_${h}H_${wv}_${ne}.nc"
...
for i in "${!ensbl[@]}"; do
  ...
  arqn="REWPS_wave_"${DATE}${CHOUR}.${ne}
  ncrcat rewps.wave.${DATE}T${CHOUR}Z_*H_${ne}.nc ${arqn}.saux.nc
  rm -f rewps.wave.${DATE}T${CHOUR}Z_*H_${ne}.nc
...
echo " "
echo " Done download_CMC_REWPS_wave.sh ${DATE} ${CHOUR}Z "

...
echo " "
echo " Looking for ECMWF cycle ${YEAR}${MONTH}${DAY}${HOUR} ..."
echo " Expected file: $MDIR/wave/$HOUR/ECMWF_ENS_wave_$YEAR$MONTH$DAY$HOUR.50.nc"
echo " "
...
while [ "$FSIZE" -lt 300000000 ] && [ "$TRIES" -le 144 ]; do
  ...
  if [ -f "$MDIR/wave/$HOUR/ECMWF_ENS_wave_$YEAR$MONTH$DAY$HOUR.50.nc" ]; then
    TE=0
  else
    TE=1
  fi
  if [ ${TE} -eq 1 ]; then
    ...
    FSIZE=$(du -sb "$MDIR/wave/$HOUR/ECMWF_ENS_wave_$YEAR$MONTH$DAY$HOUR.50.nc" | awk '{print $1}')
  fi
...
if [ "$FSIZE" -lt 300000000 ]; then
  echo " "
  echo " WARNING: gave up after ${TRIES} tries (~12h) - ECMWF file never reached expected size."

...
##########################
# probmaps_ecmwf.sh 
# based on probmaps_envcanada.sh, adapted for Environment Canada (CMCE) data
##########################
...
while [ "$FSIZE" -lt 300000000 ] && [ "$TRIES" -le 144 ]; do
  ...
  test -f $MDIR/GEWPS_wave_$YEAR$MONTH$DAY$HOUR.20.nc
  TE=$?
  if [ ${TE} -eq 1 ]; then
    FSIZE=0
  else
    FSIZE=$(du -sb "$MDIR/GEWPS_wave_$YEAR$MONTH$DAY$HOUR.20.nc" | awk '{print $1}')
  fi

...
PURPOSE:
 ...
 The goal is to provide a spatial validation on the size/position of the polygons, predicted versus occured.
...
USAGE:
 ...
 Example (from linux terminal command line):
  ...
  nohup python3 probmaps_gefs_valdt.py probmaps_gefs_internal.yaml 2023060900 7 14 Hs >> nohup_probmaps_gefs.out 2>&1 &
    ...
    for ct in range(0,len(ctime)):
        ...
        for t in range(0,auxlt.shape[0]):
            for enm in range(0,nenm):
                ...
                fname=gefspath+"GEFSv12Waves_"+cdate+"/gefs.wave."+cdate+"."+str(int(enm)).zfill(2)+".global.0p25.f"+str(int(auxlt[t])).zfill(3)+".grib2"
...
if __name__ == "__main__":
    ...
    for t in range(0,auxltime.shape[0]):
        for enm in range(0,nenm):
            ...
            fname=gefspath+"GEFSv12Waves_"+fcdate+"/gefs.wave."+fcdate+"."+str(int(enm)).zfill(2)+".global.0p25.f"+str(int(auxltime[t])).zfill(3)+".grib2"
            if c==0:
                ds = xr.open_dataset(fname, engine='cfgrib')
                wtime = np.atleast_1d(np.array(ds.time.values))
                lat = np.array(ds.latitude.values); lat = np.sort(lat); lon = np.array(ds.longitude.values)
